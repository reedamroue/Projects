...
(* This is the type of thickness and it takes in an integer which denotes the 
thickness of the line later to be used with the set_line_width function. *)
type thickness = int

(* This defines the thin and thick pen parameters for use later on *)
(* let thinpen : thickness = {th = 0}
let thickpen : thickness = {th = 5} *)
...
type gctx = {
  ...
  thickness: int (* The thickness of the pen *)
  }
...
(** Set the OCaml graphics library's internal state according to the
   Gctx settings.  Initially, this just sets the current pen color, but now
   it also sets the line thickness. *)
(* TODO: You will need to modify this definition for Task 5. *)
let set_graphics_state (gc: gctx) : unit =
  let tpen = gc.thickness in
  let c = gc.color in
  (Graphics.set_color (Graphics.rgb c.r c.g c.b));
  Graphics.set_line_width tpen
