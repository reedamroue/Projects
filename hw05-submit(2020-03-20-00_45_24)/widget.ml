...
let make_controller (v: 'a) : 'a value_controller =
  ...
  let state_list : 'b ref = {contents = []} in 
  { add_change_listener = (fun x -> state_list.contents <- 
                                                      x :: state_list.contents);
    get_value = (fun () -> state_val.contents);
    change_value = (fun v -> (state_val.contents <- v);
                             (List.iter (fun x -> x state_val.contents)
                              state_list.contents);
                    );
   }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  let val_cont = make_controller init in 
  {repaint = (fun (g:Gctx.gctx) -> if not (val_cont.get_value ()) then 
                                      ((Gctx.draw_string g (35, 0) s);
                                       (Gctx.draw_rect g (0, 0) (30, 20)))
                                    else 
                                      ((Gctx.draw_string g (35, 0) s);
                                       (Gctx.fill_rect g (0, 0) (30, 20)))
              );
   handle = (fun (g:Gctx.gctx) (e: Gctx.event) -> 
                                           begin match Gctx.event_type e with
                                             | MouseDown -> 
                                                   val_cont.change_value
                                                    (not(val_cont.get_value ()))
                                             | _ -> () 
                                           end);
   size = (fun () -> (127, 20));
  }, 
...
(* I implemented a slider generic function that takes the mouse position and 
makes a sliding bar move accordingly. To do this I made the slider take in an
integer which corresponds to the x position of the mouse. The slider works
similar to the checkbox in that it uses a value controller. The function draws
a filled rectangle according to the x position of the mouse in the gctx, and
this changes according to where the mouse is pressed and if it is dragged along
the slider bar *)
let slider (init: int) : widget * int value_controller = 
  let val_cont = make_controller init in 
  {repaint = (fun (g: Gctx.gctx) -> (Gctx.draw_rect g (0, 0) (150, 20));
                                    (Gctx.fill_rect g (0, 0) 
                                                   (val_cont.get_value (), 20));
             );
