...
type shape = 
  | Line of {thickness: int; color: color;  p1: point; p2: point}
  | Points of {thickness: int; color: color; points: point list}
  | Ellipse of {thickness: int; color: color; rx: int; ry: int; p: point}
  ...
  begin match (event_type event) with
    | MouseDown ->
      ...
      (begin match paint.mode with 
         ...
         | EllipseEndMode p1 -> ()
         | PointMode -> paint.preview <- Some 
                                   (Points {thickness = paint.thickness;
                                    color = paint.color; points = [p]})
         | LineStartMode ->
...
(*          Deque.insert_tail (Line {color=paint.color; p1=p1; p2=p}) paint.shapes;
    ...
    | MouseDrag -> begin match paint.mode with 
                      | EllipseStartMode -> paint.mode <- EllipseEndMode p
                      | EllipseEndMode p1 -> let (x, y) = p in 
                                             let (x1, y1) = p1 in 
                      paint.preview <- Some 
                     (Ellipse {thickness = paint.thickness; color = paint.color;
                     rx = (abs(x1 - x) / 2); ry = (abs(y1 - y) / 2);
                     p = (abs(x1 + x) / 2, abs(y1 + y) / 2)})
                      | PointMode -> let points = 
                                                 ...
                                                 paint.preview <- Some
                            (Points {thickness = paint.thickness; 
                             color = paint.color; points = p :: points})
                      | LineStartMode -> paint.mode <- LineEndMode p
                      | LineEndMode p1 -> paint.preview <- 
                                   Some (Line {thickness = paint.thickness;
                                         color = paint.color; p1 = p1; p2 = p})
                    end
    ...
    | MouseUp ->
      ...
      (begin match paint.mode with 
        ...
        | EllipseEndMode p1 -> let (x, y) = p in 
                               let (x1, y1) = p1 in 
                               Deque.insert_tail 
                                  (Ellipse {thickness = paint.thickness;
                                  color = paint.color; rx = (abs(x1 - x) / 2);
                                  ry = (abs(y1 - y) / 2);
                                  p = (abs(x1 + x) / 2, abs(y1 + y) / 2)})
                              paint.shapes;
        ...
        | PointMode -> let points = 
                       ...
                       Deque.insert_tail 
                                (Points {thickness = paint.thickness;
                                 color = paint.color; points = points}) 
                                 paint.shapes
        | LineStartMode -> paint.mode <- LineEndMode p
        | LineEndMode p1 -> Deque.insert_tail 
                              (Line {thickness = paint.thickness;
                              color = paint.color;
                              p1 = p1; p2 = p})
                              paint.shapes;
                             paint.mode <- LineStartMode;
...
(* Create the line, point, and ellipse buttons *)
let (wp_button, lcp_button, ncp_button ) = button "Point"

let (wl_button, lcl_button, ncl_button) = button "Line"

let (we_button, lce_button, nce_button) = button "Ellipse"

(* Creates the functionality of the thickness button and the condition
   for changing the thickness. It also calls the slider function to get 
   the value controller for use. (SEE COMMENT BELOW FOR INTERACTION INFO)   *)
 let (wid_s, vcon_s) = slider 1
 let (wid, vcon) = checkbox false "Thickness" 
 ;; vcon.add_change_listener (fun x -> if x then ((paint.thickness <- 9);
                                                 (vcon_s.change_value 9))
                                       else if not x then paint.thickness <- 
                                                            vcon_s.get_value ())  

(* Creates the functionality of the slider bar and the condition for how the 
thickness changes when you move the bar. I made the thickness change according
to the x value. Note that the slider and the button interact with eachother. 
The button puts the thickness at a constant 9 when toggled. So when you toggle 
the button the thickness automatically shifts to 9 and the slider moves to 9. 
IF YOU TRY TO MOVE THE SLIDER WHEN THE BUTTON IS TOGGLED NOTHING HAPPENS. 
This is because the button keeps the thickness at 9. However, if you untoggle
the button the thickness will automatically become whatever the thickness is 
on the slider. If you want a thin thickness just slide the slider to the very
end when the thickness button is not toggled. The thickness automatically starts
at the lowest when the button is not toggled*)
;; vcon_s.add_change_listener (fun x -> if not (vcon.get_value ()) then 
                                            paint.thickness <- x
                                        else if vcon.get_value () then 
                                            ())

(* Create the functionality of the line, point, and ellipse buttons  *)
let point_button () : unit = 
  paint.mode <- PointMode
;; ncp_button.add_event_listener (mouseclick_listener point_button)

let line_button () : unit = 
  paint.mode <- LineStartMode
;; ncl_button.add_event_listener (mouseclick_listener line_button)

let ellipse_button () : unit = 
  paint.mode <- EllipseStartMode
;; nce_button.add_event_listener (mouseclick_listener ellipse_button)
...
let mode_toolbar : widget = hlist 
          [w_undo; spacer; wp_button; spacer; wl_button; spacer; we_button;
                                                     spacer; wid; spacer; wid_s]
